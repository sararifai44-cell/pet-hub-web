      ...
      <CardContent className="p-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[11px] font-normal text-[#2F2A24]/60 line-clamp-2">
            {p?.description || (isAr ? "لا يوجد وصف" : "No description")}
          </p>
        </div>
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="text-[10px] font-medium text-[#2F2A24]/55 uppercase tracking-wide">
              {isAr ? "السعر" : "Price"}
            </div>
            <div className="text-[15px] font-semibold text-[#3C7A57] tabular-nums">
              {money(p?.price)}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Button
              asChild
              variant="outline"
              className="h-8 rounded-md border-[#E7DCD0] bg-white hover:bg-[#FBF7F1] px-3 text-[11px] font-medium"
            >
              <Link to={`/shop/${p.id}`}>{isAr ? "تفاصيل" : "Details"}</Link>
            </Button>

            <Button
              size="sm"
              className="h-8 rounded-md bg-[#3C7A57] text-white hover:bg-[#2F5F43] px-3 text-[11px] font-medium disabled:opacity-50"
              onClick={() => onAdd(p)}
