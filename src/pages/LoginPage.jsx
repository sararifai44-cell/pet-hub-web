...
export default function LoginPage() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      toast.error("Login Failed", { description: err?.data?.message || "Check your credentials." });
    }
  };
          ...
          <div className="max-w-[300px] mx-auto w-full">
            ...
            <form onSubmit={onSubmit} className="space-y-5" noValidate>
              ...
              <div className="text-left">
                <label htmlFor="email" className="mb-2 block text-[10px] font-bold uppercase tracking-widest text-[#94a3b8]">
                  Email Address
                </label>
                <input
                  id="email"
                  ...
                  disabled={isLoading}
                />
                {showEmailError && <p className="mt-1.5 text-[10px] text-red-500 font-medium">*{errors.email}</p>}
              </div>

              {/* Password Section (Left Aligned) */}
              <div className="text-left">
                <div className="flex items-center justify-between mb-2">
                  <label htmlFor="password" className="block text-[10px] font-bold uppercase tracking-widest text-[#94a3b8]">
                    Password
                  </label>
                  <Link to="/forgot-password" size="sm" className="text-[10px] font-bold text-[#E07060] hover:underline">
                    Forgot?
                  </Link>
                </div>
                <input
                  ...
                  disabled={isLoading}
                />
                {showPasswordError && <p className="mt-1.5 text-[10px] text-red-500 font-medium">*{errors.password}</p>}
              </div>
            ...
            <p className="mt-8 text-center text-xs font-medium text-[#64748b]">
              New here? <Link to="/register" className="font-bold text-[#E07060] hover:underline decoration-2 underline-offset-4">Create account</Link>
            </p>
          </div>
        </div>
