...
export default function BoardingPage() {
  ...
  const services = useMemo(
    () =>
      Array.isArray(servicesRes?.data)
        ? servicesRes.data.filter((s) => s.is_active !== false)
        : [],
    [servicesRes]
  );
  ...
  const [getQuote, { data: quoteData, isLoading: quoting }] =
    useGetBoardingQuoteMutation();
  const [createReservation, { isLoading: creating }] =
    useCreateBoardingReservationMutation();

  // ✅ تعديل مهم: pet_breed_id اختياري (لا نرسله إذا فاضي)
  const payload = useMemo(
    () => {
      ...
      // إذا السلالة مختارة فعلاً فقط
      if (petBreedId) base.pet_breed_id = Number(petBreedId);
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4 max-w-lg">
                  <p className="text-[#e2f3f0] text-sm md:text-base leading-relaxed opacity-90 font-medium">
                    Our policy ensures a safe, medical-grade environment for
                    your pets. We provide 24/7 supervision and regular health
                    check-ups.
                  </p>
                  <ul className="space-y-4">
                    {[
                      {
                        text: "Daily medical screening",
                        icon: (
                          <ShieldCheck className="h-4 w-4 text-emerald-400" />
                        ),
                        desc: "Professional vet check-up every morning.",
                      },
                      {
                        text: "Climate-controlled suites",
                        icon: <Sparkles className="h-4 w-4 text-amber-300" />,
                        desc: "Perfect temperature for deep rest.",
                      },
                      {
                        text: "Supervised social playtime",
                        icon: <Heart className="h-4 w-4 text-pink-400" />,
                        desc: "Safe interaction under expert eyes.",
                      },
                    ].map((item, i) => (
                      <li key={i} className="flex items-start gap-3 group">
                        <div className="mt-1 p-1.5 rounded-lg bg-white/5 border border-white/10 group-hover:scale-110 transition-transform">
                          {item.icon}
                        </div>
                        <div>
                ...
                <div className="flex flex-wrap gap-4 pt-4">
                  <button
                    ...
                    className="px-8 py-4 rounded-xl bg-[#a8d5cb] text-[#345e54] text-sm font-black hover:bg-white transition-all shadow-lg flex items-center gap-2"
                  >
                    Book Now <ArrowLeft className="rotate-[270deg] w-4 h-4" />
                  </button>
                  <button
                    onClick={() => navigate("/")}
                    className="px-8 py-4 rounded-xl bg-transparent border-2 border-white/20 text-white text-sm font-bold hover:bg-white/10 transition-all"
                  >
                    Home Page
                  </button>
                </div>
            ...
            <div className="lg:col-span-2 bg-[#fcfaf7] rounded-2xl shadow-xl border border-[#d6ccbd] overflow-hidden">
              <div className="p-8 md:p-12">
                <div className="flex items-center gap-5 mb-10 pb-6 border-b border-[#e8dfd1]">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-[#6d4c41] tracking-tight">
                      Reservation Details
                    </h2>
                    <p className="text-[10px] text-[#6d4c41]/50 font-bold uppercase tracking-widest mt-1">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="space-y-6">
                    <h3 className="text-[11px] font-bold text-[#6d4c41] uppercase tracking-[0.2em] flex items-center gap-2">
                      <ScrollText className="w-4 h-4 opacity-50" /> Pet Info
                    </h3>
                    <div className="grid gap-5">
                      <div className="relative">
                        <label className="text-[9px] font-bold text-[#6d4c41] uppercase absolute -top-2 left-4 bg-[#fcfaf7] px-2 z-10">
                          Pet Type
                        </label>
                        <select
                          value={petTypeId}
                          ...
                          className="w-full border border-[#d6ccbd] rounded-xl px-5 py-4 text-sm font-medium text-[#6d4c41] focus:border-[#6d4c41] outline-none bg-white transition-all"
                        >
                      ...
                      <div className="relative">
                        ...
                        <label className="text-[9px] font-bold text-[#6d4c41] uppercase absolute -top-2 left-4 bg-[#fcfaf7] px-2 z-10 flex items-center gap-2">
                          <span>Breed</span>
                          <span className="text-[9px] font-black text-[#345e54] bg-[#345e54]/10 px-2 py-0.5 rounded-full">
                            Optional
                          </span>
                        </label>
                        ...
                        <select
                          ...
                          disabled={!petTypeId}
                          className="w-full border border-[#d6ccbd] rounded-xl px-5 py-4 text-sm font-medium text-[#6d4c41] focus:border-[#6d4c41] outline-none bg-white disabled:opacity-50 transition-all"
                        >
                  ...
                  <div className="space-y-6">
                    <h3 className="text-[11px] font-bold text-[#6d4c41] uppercase tracking-[0.2em] flex items-center gap-2">
                      <Clock className="w-4 h-4 opacity-50" /> Schedule
                    </h3>
                    <div className="grid gap-5">
                      <div className="relative">
                        <label className="text-[9px] font-bold text-[#6d4c41] uppercase absolute -top-2 left-4 bg-[#fcfaf7] px-2 z-10">
                ...
                <div className="mt-12 space-y-6">
                  <h3 className="text-[11px] font-bold text-[#6d4c41] uppercase tracking-[0.2em]">
                    Extra Care Services
                  </h3>
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${
                            active
                              ? "bg-[#6d4c41] border-[#6d4c41] text-white shadow-md"
                              : "bg-white border-[#d6ccbd] text-[#6d4c41]"
                          }`}
                        >
                          <div className="text-left">
                            ...
                            <p
                              className={`text-[10px] mt-1 ${
                                active ? "text-white/70" : "text-[#6d4c41]/50"
                              }`}
                            >
                              {money(s.price)}
                            </p>
                          ...
                          {active ? (
                            ...
                            <div className="h-5 w-5 rounded-full border border-[#d6ccbd]" />
                          )}
                        </button>
                      );
                    })}
                ...
                <div className="mt-12 pt-8 border-t border-[#e8dfd1]">
                  <button
                    ...
                    disabled={quoting}
                    className="w-full py-5 rounded-xl bg-[#6d4c41] text-white text-base font-bold hover:bg-[#543b32] transition-all shadow-lg flex items-center justify-center gap-3"
                  >
                    {quoting ? (
                      <CircularProgress size={22} color="inherit" />
                    ) : (
                      "Calculate My Pet's Stay"
                    )}
                  </button>
                </div>
              </div>
                  ...
                  {[
                    {
                      ...
                      text: "Up-to-date vaccination records are mandatory upon check-in.",
                      icon: <VerifiedUserIcon sx={{ fontSize: 16 }} />,
                    },
                    {
                      title: "Check-in Window",
                      text: "Arrival is required 15 minutes before your slot.",
                      icon: <AccessTimeFilledIcon sx={{ fontSize: 16 }} />,
                    },
                    {
                      title: "Cancellation",
                      text: "Cancel for free up to 24 hours before your pet’s stay.",
                      icon: <CancelScheduleSendIcon sx={{ fontSize: 16 }} />,
                    },
                    {
                      title: "Emergency",
                      text: "Please provide an active emergency number.",
                      icon: <ContactPhoneIcon sx={{ fontSize: 16 }} />,
                    },
                  ].map((item, i) => (
                    <li key={i}>
          ...
          <div className="space-y-6 bg-[#f5f0e8] rounded-2xl p-8 mb-10 border border-[#d6ccbd]">
            ...
            <div className="flex justify-between items-center text-base border-b border-[#d6ccbd]/50 pb-5">
              ...
              <span className="font-bold text-[#6d4c41] text-lg">
                {quoteData?.billable_hours || 0} hrs
              </span>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <LocalActivityIcon
                  sx={{ fontSize: 18, color: "#6d4c41", opacity: 0.5 }}
                />
                <p className="text-[11px] font-bold text-[#6d4c41]/50 uppercase tracking-wider">
              ...
              {services.filter((s) => !!selectedServices[s.id]).length > 0 ? (
                <div className="space-y-2">
                  {services
                    ...
                    .map((service) => (
                      <div
                        key={service.id}
                        className="flex justify-between items-center bg-white/60 p-4 rounded-xl border border-[#d6ccbd]/30 hover:bg-white transition-colors"
                      >
                        <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-black text-[#345e54]">
                          {money(service.price)}
                        </span>
                      </div>
                    ))}
                ...
                <div className="py-4 text-center border-2 border-dashed border-[#d6ccbd]/40 rounded-xl">
                  <p className="text-xs italic text-[#6d4c41]/40">
            ...
            <div className="pt-6 border-t border-[#d6ccbd] flex justify-between items-end">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <PaymentsIcon
                    sx={{ fontSize: 16, color: "#345e54", opacity: 0.6 }}
                  />
                  <p className="text-[11px] font-bold text-[#6d4c41]/50 uppercase tracking-wider">
                    Total Amount
                  </p>
